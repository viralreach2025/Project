...
-- This policy allows insertion when the id matches the authenticated user's id
-- OR when inserting a new user profile (auth.uid() will be null during signup)
...
DROP POLICY IF EXISTS "Enable insert for all users" ON public.waitlist_entries;
